...
# Since this lab uses simulated data you could very easily use the whole population to draw conclusions from the data. For instance if you didn't know about the values of 𝜇
#  and 𝜎
#  you could get very close estimates of the true values by computing the mean and standard deviation of the whole population like so:
...
# The first one is to compute the theoretical 𝜇
#  and 𝜎
#  of the sample means which will be denoted with the symbols 𝜇𝑋¯
#  and 𝜎𝑋¯
#  respectively. These values can be computed as follows:
...
# And then use these values to plot a Gaussian curve with parameters 𝜇𝑋¯
#  and 𝜎𝑋¯
# . If the theorem holds then the resulting distribution of the sample means should resemble this Gaussian curve. Run the next cell to include this into the plot:
...
# Now, put everything together in an interactive widget to experiment with different values for 𝜇
# , 𝜎
#  and sample_size. To update the plots you will need to click the Run Interact button after changing the parameters:
...
As with the previous distribution, by running the next cell you will launch an interactive widget in which you can play around with different values of 𝑛
, 𝑝
 and 𝑠𝑎𝑚𝑝𝑙𝑒_𝑠𝑖𝑧𝑒
.

See if you can find anything interesting, for instance does the theorem seem to hold better when 𝑝
 is close to 0.5?
...
Poisson Population
Another popular distribution you might have heard of is the poisson distribution. It models the number of events occurring in a fixed interval of time or space, given the average rate of occurrence 𝜇
 of those events.
